...
const RecentActivity = () => {
  ...
  const fetchActivities = async (showLoading = true) => {
    ...
    try {
      ...
      const activitiesRes = await fetch(`https://online-auction-backend-mkn1.onrender.com/api/user-activities/${user.email}`);
      ...
      if (activitiesData.status === 'success') {
        ...
        const [postedRes, bidsRes, profileRes, paymentsRes, notificationsRes] = await Promise.all([
          fetch(`https://online-auction-backend-mkn1.onrender.com/api/items/user/${user.email}`),
          fetch(`https://online-auction-backend-mkn1.onrender.com/my-bids/email/${user.email}`),
          fetch(`https://online-auction-backend-mkn1.onrender.com/api/get-profile?email=${user.email}`),
          fetch(`https://online-auction-backend-mkn1.onrender.com/api/payments/${user.email}`),
          fetch(`https://online-auction-backend-mkn1.onrender.com/api/notifications/${user.email}`)
        ]);
  ...
  const handleFilterClick = async (newFilter) => {
    ...
    try {
      ...
      const populateRes = await fetch(`https://online-auction-backend-mkn1.onrender.com/api/populate-activities/${user.email}`, {
        method: 'POST',
